...
/* ASYNC constants */
#define ASYNC_QUEUE_NAME "lights_piix4"
...
static void piix4_adapter_destroy (struct hw_device *hw_dev);

const struct hw_device_vtable smbus_vtable = {
    .release        = piix4_adapter_destroy,
    .read           = smbus_read,
    .write          = smbus_write,
    .read_async     = smbus_read_async,
    .write_async    = smbus_write_async,
};
...
struct smbus_context {
    struct smbus_adapter    smbus_adapter;
    struct i2c_adapter      adapter;
    uint16_t                smba;
};
#define ctx_from_adapter(ptr) \
    container_of(to_smbus_adapter(ptr), struct smbus_context, smbus_adapter)

static int smbus_piix4_transaction (
    struct i2c_adapter *adapter
){
    struct smbus_context *ctx = i2c_get_adapdata(adapter);
...
static int smbus_piix4_transfer (
    ...
    status = smbus_piix4_transaction(adapter);
    if (status)
        return status;
...
static struct smbus_context *smbus_piix4_context_create (
    ...
    if (!request_muxed_region(SB800_PIIX4_SMB_IDX, 2, MUXED_NAME)) {
        LIGHTS_HW_ERR("SMB base address index region 0x%x already in use.", SB800_PIIX4_SMB_IDX);
    ...
    if (!smb_en_status) {
        LIGHTS_HW_ERR("SMBus Host Controller not enabled!");
    ...
    LIGHTS_HW_INFO("Auxiliary SMBus Host Controller at 0x%x", smba);

    ctx = kzalloc(sizeof(*ctx), GFP_KERNEL);
    if (!ctx)
        return ERR_PTR(-ENOMEM);

    queue = async_queue_create(ASYNC_QUEUE_NAME, ASYNC_QUEUE_SIZE);
    if (!queue) {
        kfree(ctx);
        return ERR_PTR(-ENOMEM);
    }

    ctx->smbus_adapter.hw_dev.vtable = &smbus_vtable;
    ctx->smbus_adapter.async_queue = queue;
    ctx->smba = smba;
...
static void piix4_adapter_destroy (
    struct hw_device *hw_dev
){
    struct smbus_context *context = ctx_from_adapter(hw_dev);

    if (WARN_ON(NULL == hw_dev))
        return;

    i2c_del_adapter(&context->adapter);
    async_queue_destroy(context->smbus_adapter.async_queue);

    kfree(context);
}

struct hw_device *piix4_adapter_create (
    void
){
    ...
    context->smbus_adapter.i2c_adapter = &context->adapter;
    // context->smbus_adapter.destroy = smbus_piix4_adapter_destroy;

    return &context->smbus_adapter.hw_dev;
}
